...
@systems.register('pinhole-opt-neus-system')
class PinholeNeuSSystem(BaseSystem):
    ...
    def prepare(self):
        ...
        self.views_list = [int(i * 4) for i in range(8)]
        self.views_tensor = torch.tensor(self.views_list)

    def forward(self, batch):
        return self.model(batch['rays'])

    def preprocess_data(self, batch, stage, step=None):
        periods= [(500, 1000), (1500, 2000), (2500, 3000), (3500, 4000)]
        if 'index' in batch:  # validation / testing
            ...
            if self.config.model.batch_image_sampling:
                if sample_all:
                    ...
                    index = torch.randint(0, len(self.views_tensor), size=(self.train_num_rays,))
                    index = self.views_tensor[index].to(self.dataset.all_images.device)
        ...
        if stage in ['train']:
            ...
            normal = self.dataset.all_normals_world[index, y, x].view(-1, self.dataset.all_normals_world.shape[-1]).to(
                self.rank)
    ...
    def training_step(self, batch, batch_idx):
        ...
        view_idx = batch['view_idx']
        # print(view_idx.shape)

        idx_mask = torch.zeros_like(view_idx, dtype=torch.bool)

        # Iterate over self.views_tensor to set True in index_mask where index matches
        for view_index in self.views_tensor:
            idx_mask |= (view_idx == view_index)
        ...
        loss_normal = ranking_loss(normal_errors[mask], penalize_ratio=self.config.system.loss.normal_p_ratio,
        ...
        loss_mask = binary_cross_entropy(opacity[idx_mask], batch['fg_mask'][idx_mask].float(), reduction='none')
        loss_mask = ranking_loss(loss_mask, penalize_ratio=self.config.system.loss.mask_p_ratio,
        ...
        if self.config.model.learned_background and self.C(self.config.system.loss.lambda_distortion_bg) > 0:
            loss_distortion_bg = flatten_eff_distloss(out['weights_bg'], out['points_bg'], out['intervals_bg'],
                                                      out['ray_indices_bg'])
